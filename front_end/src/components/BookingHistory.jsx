...
const BookingHistory = () => {
    ...
    useEffect(()=>{
        ...
        axios.get("http://localhost:3011/getBookedRoomData")
        .then(user => {
            setBookedRoom(user.data)
        })
        .catch(err => {
            console.log(err)
        })
    ...
    return(
     ...
     <nav class="bg-[#121e31] w-[30%] h-screen fixed top-0 left-0 min-w-[250px] py-6 px-4 font-[sans-serif] tracking-wide overflow-auto">
      ...
      <ul class="space-y-3">
        ...
        <li>
          <Link to="/bookinghistory" class="text-white text-sm flex items-center hover:bg-gray-700 rounded px-4 py-3 transition-all">
            <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="w-[18px] h-[18px] mr-4"
              viewBox="0 0 512 512">
              <path
...
<td class="p-4 text-[15px] text-gray-800">
{
    book.bookedDate.map((bookroom)=>{
        return bookroom.checkinDate
    })
